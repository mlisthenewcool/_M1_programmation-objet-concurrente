...
public class Parallèle extends Thread {
	static long nbTirages = 800_000_000;          // Précision du calcul, fixée à 5 000 000
	volatile long tiragesDansLeDisque = 0;      // Nb de tirages dans le disque pour chaque thread
	static int nbThreads = 8;                   // Nb de threads utilisés
	long part;

	public static void main (String args[]) {
		if (args.length>0) {
			try {
				nbTirages = 1_000_000 * Integer.parseInt(args[0]);
				nbThreads = Integer.parseInt(args[1]);
			}
			catch(NumberFormatException nfe) {
				System.err.println
					("Usage : java Parallèle <nb de tirages> <nb de threads>");
		...
		System.out.format("Nombre de threads %d.%n", nbThreads);
		final long startTime = System.nanoTime();
		final long endTime;

		Parallèle[] T = new Parallèle[nbThreads];
		for(int i=0; i<nbThreads; i++){
			T[i] = new Parallèle(nbTirages / nbThreads);
			T[i].start();
		}
		for(int i=0; i<nbThreads; i++){
